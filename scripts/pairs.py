...
def get_base_atoms(res: Bio.PDB.Residue.Residue) -> Optional[Tuple[Bio.PDB.Atom.Atom, List[Bio.PDB.Atom.Atom]]]:
    if "C1'" in res:
        c1 = res["C1'"]
...
def get_residue_posinfo(res: Bio.PDB.Residue.Residue) -> ResiduePosition:
    """
    ...
    if x is None:
        raise ResideTransformError(f"No C1' atom found at {res.full_id}")
    c1, planar_atoms = x
    ...
    if np.any(np.linalg.norm(atoms, axis=1) < 1.3):
        raise ResideTransformError(f"Atoms too close to origin at {res.full_id}")
    ...
    if sum_error > 1 * len(planar_atoms): # TODO: reasonable threshold?
        raise ResideTransformError(f"Residue really doesn't seem planar: {res.full_id}, plane RMSE = {np.sqrt(sum_error / len(planar_atoms))}")
...
def try_get_residue_posinfo(res: Bio.PDB.Residue.Residue, warning = None) -> Optional[ResiduePosition]:
    try:
        ...
        if warning is not None:
            ...
            print(f"WARNING(residue = {res.full_id}): {e}")
        return None
...
def calc_pair_stats(res1: Bio.PDB.Residue.Residue, res2: Bio.PDB.Residue.Residue) -> Optional[PairStats]:
    p1 = try_get_residue_posinfo(res1)
    p2 = try_get_residue_posinfo(res2)
    if p1 is None or p2 is None:
        return None
    res1_ = transform_residue(res1, p1.translation, p1.rotation)
    res2_ = transform_residue(res2, p1.translation, p1.rotation)
...
def hbond_stats(atom0: Bio.PDB.Atom.Atom, atom1: Bio.PDB.Atom.Atom, atom2: Bio.PDB.Atom.Atom, atom3: Bio.PDB.Atom.Atom) -> HBondStats:
    ...
    assert np.linalg.norm(atom1.coord - atom2.coord) < 4.5, f"atoms too far for h-bond: {atom1.full_id} {atom2.full_id} ({np.linalg.norm(atom1.coord - atom2.coord)} > 4, {atom1.coord} {atom2.coord})"
    return HBondStats(
        get_distance(atom1, atom2),
        get_angle(atom0, atom1, atom2),
        get_angle(atom3, atom2, atom1)
    )
...
def get_hbond_stats(pair_type: str, r1: Bio.PDB.Residue.Residue, r2: Bio.PDB.Residue.Residue) -> Optional[List[HBondStats]]:
    ...
    return [ hbond_stats(get_atom(a0), get_atom(a1), get_atom(a2), get_atom(a3)) for a0, a1, a2, a3 in hbonding_atoms[(pair_type, pair_name)] ]
...
def get_stats_for_csv(df_: pl.DataFrame, structure: Bio.PDB.Structure.Structure):
    df = df_.with_row_count()
    for (i, pdbid, model, chain1, res1, chain2, res2) in zip(df["row_nr"], df["pdbid"], df["model"], df["chain1"], df["nr1"], df["chain2"], df["nr2"]):
        try:
            assert structure.id.lower() == pdbid.lower(), f"pdbid mismatch: {structure.id} != {pdbid}"

            try:
                r1 = structure[model-1][chain1][res1]
            except KeyError:
                print(f"Could not find residue1 {chain1}.{res1} in {pdbid}")
                continue
            try:
                r2 = structure[model-1][chain2][res2]
            except KeyError:
                print(f"Could not find residue2 {chain2}.{res2} in {pdbid}")
                continue
            hbonds = get_hbond_stats("cWW", r1, r2)
            stats = None # calc_pair_stats(r1, r2)
            if hbonds is not None:
                yield i, hbonds, stats
        except AssertionError as e:
            print(f"{e} on row {next(df_[i, :].iter_rows())}")
            continue
        except Exception as e:
            print(f"Error on row {next(df_[i, :].iter_rows())}")
            import traceback
            print(e)
...
def export_stats_csv(pdbid, df: pl.DataFrame, add_metadata_columns: bool) -> Tuple[str, pl.DataFrame, np.ndarray]:
    bond_count = 3
    ...
    if structure is not None:
        for i, hbonds, stats in get_stats_for_csv(df, structure):
            valid[i] = True
...
if __name__ == "__main__":
    ...
    parser.add_argument("--metadata", action="store_true", default=False, help="Add deposition_date, resolution and structure_method columns")
    ...
    with multiprocessing.Pool(processes=args.threads) as pool:
        ...
        processes.append([
            pool.apply_async(export_stats_csv, args=[pdbid, group, args.metadata])
