<script lang="ts">
    ...
    function rerender() {
        ...
        const columns = settings.variables.map(v => a.getColumnHelper(data, v))
        let min = Math.min(...columns.map(c => c.data.reduce((a, b) => Math.min(a, b), 1e50)))
        let max = Math.max(...columns.map(c => c.data.reduce((a, b) => Math.max(a, b), -1e50)))
        ...
        for (let i = 0; i < columns.length; i++) {
            ...
            node.append("g")
                ...
                .append("title")
                .text(d => `${d.x0} <= [${settings?.variables[i]?.label}] <= ${d.x1}: ${d.count}`)
        }
